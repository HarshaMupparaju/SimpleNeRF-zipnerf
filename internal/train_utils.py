...
def compute_data_loss(batch, renderings, config):
    ...
    lossmult = lossmult[indices_mask].reshape(config.batch_size, 1, 1, -1)
    ...
    for rendering in renderings:
        ...
        rendering_rgb = rendering['rgb'][indices_mask].reshape(config.batch_size, 1, 1, -1)
        batch_rgb = batch['rgb'][indices_mask].reshape(config.batch_size, 1, 1, -1)
        ...
        if config.compute_disp_metrics:
            ...
            # be used instead.
            rendering_distance_mean = rendering['distance_mean'][indices_mask].reshape(config.batch_size, 1, 1, -1)
            batch_disps = batch['disps'][indices_mask].reshape(config.batch_size, 1, 1, -1)
            disp = 1 / (1 + rendering_distance_mean)
            stats['disparity_mses'].append(((disp - batch_disps) ** 2).mean().item())

        if config.compute_normal_metrics:
            if 'normals' in rendering:
                batch_alphas = batch['alphas'][indices_mask].reshape(config.batch_size, 1, 1, -1)
                batch_normals = batch['normals'][indices_mask].reshape(config.batch_size, 1, 1, -1)
                rendering_normals = rendering['normals'][indices_mask].reshape(config.batch_size, 1, 1, -1)
                rendering_acc = rendering['acc'][indices_mask].reshape(config.batch_size, 1, 1, -1)
                weights = rendering_acc * batch_alphas
                normalized_normals_gt = ref_utils.l2_normalize(batch_normals)
                normalized_normals = ref_utils.l2_normalize(rendering_normals)
...
def interlevel_loss(batch, ray_history, config):
    """Computes the interlevel loss defined in mip-NeRF 360."""
    indices_mask = batch['original_batch_mask'].reshape(-1).bool()
    # Stop the gradient from the interlevel loss onto the NeRF MLP.
    last_ray_results = ray_history[-1]
    c = last_ray_results['sdist'][indices_mask].reshape(config.batch_size, 1, 1, -1).detach()
    w = last_ray_results['weights'][indices_mask].reshape(config.batch_size, 1, 1, -1).detach()
    loss_interlevel = 0.
    for ray_results in ray_history[:-1]:
        cp = ray_results['sdist'][indices_mask].reshape(config.batch_size, 1, 1, -1)
        wp = ray_results['weights'][indices_mask].reshape(config.batch_size, 1, 1, -1)
        loss_interlevel += stepfun.lossfun_outer(c, w, cp, wp).mean()
    return config.interlevel_loss_mult * loss_interlevel


def anti_interlevel_loss(batch, ray_history, config):
    """Computes the interlevel loss defined in mip-NeRF 360."""
    indices_mask = batch['original_batch_mask'].reshape(-1).bool()
    last_ray_results = ray_history[-1]
    c = last_ray_results['sdist'][indices_mask].reshape(config.batch_size, 1, 1, -1).detach()
...
def distortion_loss(batch, ray_history, config):
    """Computes the distortion loss regularizer defined in mip-NeRF 360."""
    indices_mask = batch['original_batch_mask'].reshape(-1).bool()
    last_ray_results = ray_history[-1]
    c = last_ray_results['sdist'][indices_mask].reshape(config.batch_size, 1, 1, -1)
    w = last_ray_results['weights'][indices_mask].reshape(config.batch_size, 1, 1, -1)
...
def orientation_loss(batch, model, ray_history, config):
    ...
    for i, ray_results in enumerate(ray_history):
        w = ray_results['weights'][indices_mask].reshape(config.batch_size, 1, 1, -1)
        n = ray_results[config.orientation_loss_target]
        if n is None:
            raise ValueError('Normals cannot be None if orientation loss is on.')
        else:
            n = n[indices_mask].reshape(config.batch_size, 1, 1, -1)
        # Negate viewdirs to represent normalized vectors from point to camera.
        v = -1. * batch['viewdirs'][indices_mask].reshape(config.batch_size, 1, 1, -1)
...
def opacity_loss(batch, renderings, config):
    indices_mask = batch['original_batch_mask'].reshape(-1).bool()
    total_loss = 0.
    for i, rendering in enumerate(renderings):
        o = rendering['acc'][indices_mask].reshape(config.batch_size, 1, 1)
        total_loss += config.opacity_loss_mult * (-o * torch.log(o + 1e-5)).mean()
    return total_loss


def predicted_normal_loss(batch, model, ray_history, config):
    ...
    for i, ray_results in enumerate(ray_history):
        ...
        if n is None or n_pred is None:
            ...
            n = n[indices_mask].reshape(config.batch_size, 1, 1, -1)
            n_pred = n_pred[indices_mask].reshape(config.batch_size, 1, 1, -1)
...
# def penalize_depth_loss(renderings, config):
#     total_loss = 0
#     max_depth = 8
#     # global depth range
#     # p = config.render_dist_percentile
#     # distance_limits = np.percentile(renderings[-1]['depth'].flatten(), [p, 100 - p])
#     # lo, hi = distance_limits
#     # print(lo, hi)
#     # print(renderings[-1]['depth'].shape)
#     # print(1/0)
    
#     for i, rendering in enumerate(renderings):
#         depth = rendering['depth']

#         total_loss += torch.relu(max_depth/4 - depth)
#         # total_loss += torch.relu(5 - depth).mean()

#     total_loss = config.penalizing_depth_loss_mult * torch.mean(total_loss)

#     return total_loss

def sparse_depth_loss(batch, renderings, config):
    indices_mask = ~batch['original_batch_mask'].bool()
    total_loss = 0
    rendering = renderings[2]

    depth = rendering['sdepth'][indices_mask].reshape(config.sparse_depth_batch_size)
    gt_depth = batch['sparse_depth'][indices_mask].reshape(config.sparse_depth_batch_size)
    total_loss += torch.mean((depth - gt_depth) ** 2)
